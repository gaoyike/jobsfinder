...
class jobapply {
  def table = {
    val cols = "简历名称" :: "创建时间" :: "修改时间" :: "使用这个简历" :: Nil
    lazy val fun = (params: DataTableParams) => {
      ...
      new DataTableObjectSource(count, count, rows.map(r =>
        List(("0", r._1),
          ("1", r._2),("2", r._3),("3", (<button type="button" datatoggle="button" onClick={process(r._1)}>使用这个简历</button>).toString()),
    ...
    def process(s:String) = {
      var b = S.param("jobid").open_!
      JsCmds.RedirectTo("/sendMail.html?resumeName="+s+"&jobid="+b)
    }
